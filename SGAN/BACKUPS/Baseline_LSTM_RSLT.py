...
def plot_traindataset_result(X_train, y_train):
    ...
    train_predict_index = np.load(ModelFileDir + TrainCaseName + '_' +"train_predict_index.npy", allow_pickle=True)
    ...
    for i in range(rescaled_predicted_y.shape[0]):
        y_predict = pd.DataFrame(rescaled_predicted_y[i], columns=["predicted_PM"],
                                 index=train_predict_index[i:i + output_dim])
        predict_result = pd.concat([predict_result, y_predict], axis=1, sort=False)

    real_PM = pd.DataFrame()
    for i in range(rescaled_real_y.shape[0]):
        y_train = pd.DataFrame(rescaled_real_y[i], columns=["real_PM"],
                               index=train_predict_index[i:i + output_dim])
        real_PM = pd.concat([real_PM, y_train], axis=1, sort=False)

    predict_result['predicted_mean'] = predict_result.mean(axis=1)
    real_PM['real_mean'] = real_PM.mean(axis=1)
    #
    # Plot the predicted result
    plt.figure(figsize=(16, 8))
    plt.plot(real_PM["real_mean"])
...
def plot_testdataset_result(X_test, y_test):
    ...
    y_scaler = load(open(yScaler, 'rb'))
    test_predict_index = np.load(ModelFileDir + TrainCaseName + '_' + "test_predict_index.npy", allow_pickle=True)
