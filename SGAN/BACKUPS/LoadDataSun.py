...
sqlTextSun = "SELECT CONCAT(REG_YMD,REG_HH24,REG_MM) AS DATE," \
             "           PW_P AS PW ," \
             "           CAPACITY_MW AS CAPA" \
             "          FROM AST0203" \
             "          WHERE REG_YMD BETWEEN '20061101' AND '20070201' ;"

#dl.dataLoadSQL(sqlTextSun, "sunData.csv")

## import data
#df = pd.read_csv('windData.csv')
df = pd.read_csv('sunData.csv')
...
ax.plot(df[ 'IDX'], df['PW'], label='SUN_POWER')
ax.set(xlabel="Date",
       ylabel="MW",
       title="SUN_POWER US")
...
Final_data.to_csv("SunData_with_Fourier.csv", index=False)
