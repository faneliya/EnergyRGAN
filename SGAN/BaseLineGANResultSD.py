...
def plot_traindataset_result(RealValue, PredictedValue):
    ...
    train_predict_index = np.load(ProcessedFilesDir + TrainCaseName + "_" + DataVersion +
    ...
    plt.tight_layout()
    plt.savefig('./PICS/'+TrainCaseName + '_GAN_traindataset.png')
    plt.show()
...
def plot_testdataset_result(X_test, y_test, model):
    ...
    plt.title(TrainCaseName + " GAN : result of Training, RMSE=" + str(RMSE), fontsize=16)
    plt.tight_layout()
    plt.savefig('./PICS/'+TrainCaseName + '_GAN_testdataset.png')
    plt.show()
